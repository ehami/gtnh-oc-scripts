...
--  3. Connect all covers with enderio covered conduits. Set the machine control covers to the lime color. Set the activity covers to orange. Optionally, connect colored lamps to each channel for visibility.
...
--  5. Either flash a microcontroller with this code or run it on a computer. I'd rec a microcontroller.
...
--  8. Connect the redstone conduits to the sides configured below. Note the sides are "from the computer's point of view", and will be reversed when looking at the face of the computer. 
...
local redstoneOutputSide = sides.left
...
local redstoneInputSide = sides.right

-- Hatch Side
local fluidInputSide = sides.up

-- Dump tank/interface Side
local fluidOutputSide = sides.down
